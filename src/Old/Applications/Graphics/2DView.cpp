...
//    randBlockForward->setMaterial({GRAY, 1});
    ...
    randBlockForward2->setMaterial({DARK_BLUE, -1, 1  });
...
////LIGHTS
    ...
    int lightWidth = 20;
    lights.push_back( new SpotLight( Vec3d(0 - lightWidth,65,180 - lightWidth), Vec3d(0 + lightWidth,65,180 + lightWidth), 1));
...
void secondAudiScene( RayTracer*& rayTracer, int w, int h, const RayTracerParameters& params ) {
    double FOV = 67.38;
    double dV = w / 2 / tan( FOV * M_PI / 180 / 2 );
    Camera* cam = new Camera( Vec3d(0,10,0 ), Vec3d(0,0,1), dV,w,h );
    //Camera* cam = new Camera( Vec3d(0,0,0 ), Vec3d(0,0,1), 6000,3200,2000 );
...
////AUDI
    ...
    // mb 75 is light
    std::vector< std::pair< std::vector<int>, Material > > drawing;
    Material steel( GRAY, -1, 0.05, 1 );

    Material blackMetal( BLACK, -1, 0.05, 1 );
    Material gold( YELLOW, -1, 0.1, 1 );
    ...
    std::vector<int> bodyIndexes =
            { 0, 1, 2, 3, 4, 5, 6, 7, 8, 14, 15, 18, 21, 24, 28, 29, 30, 54, 55, 56, 69, 70, 71, 75, 83, 84, 88, 89, 106, 107, 110, 111, 114, 127, 168 };
    Material body( {110,170,255}, -1, 0.15, 1 );
    drawing.push_back( { bodyIndexes, body } );
...
//    srand(0);
    ...
    secondAudiScene( rayTracer, w, h, params );
