...
void Camera::render( const HittableList& world, unsigned char* colorBuffer ) {
    ...
    for ( int i = 0; i < imageWidth; ++i ) {
        for ( int j = 0; j < imageHeight; ++j ) {
            ...
            for ( int s = 0; s < samplesPerPixel; ++s ) {
                ...
                Ray ray = getRay( i, j );
                pixelColor += traceRay(ray, world, maxDepth );
            }
...
RGB Camera::traceRay( const Ray& ray, const HittableList& world, int depth ) {
